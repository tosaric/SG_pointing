...
def run_astrometry_plate_solver(event_path):
    ...
    return result
    # print(result.stdout.decode('utf-8'))  # Output u terminalu
...
def solve_leds(event_path, w, target_source, target_X, target_Y, scale):
    ...
    aa = AltAz(location=ORM, obstime=time)

    x, y = centroid_sources(image, x_init, y_init,
                            footprint=footprint, centroid_func=centroid_com)

    xc_1, yc_1, R_1 = circle_fit.fit_circle(x, y)
    xc_1 = (xc_1/np.cos(np.deg2rad(4.88)))
    ...
    dx_unfixed = np.tan(np.deg2rad(
        np.abs(LED_centerX - target_X.item())*scale/3600))*28*1000
    dy_unfixed = np.tan(np.deg2rad(
        np.abs(LED_centerY - target_Y.item())*scale/3600))*28*1000

    #     LST misspointing before shift
    ...
    dx_fixed = np.tan(np.deg2rad(
        np.abs(LED_centerX_fix - target_X.item())*scale/3600))*28*1000
    dy_fixed = np.tan(np.deg2rad(
        np.abs(LED_centerY_fix - target_Y.item())*scale/3600))*28*1000
...
def starfield_solve(event):
    ...
    columns_from_leds = [
        ...
        "LED_center_Alt[deg]", "LED_center_Az[deg]", "Radius[px]", "Radius[mm]",
    ...
    aa = AltAz(location=ORM, obstime=time)
    target_source = SkyCoord(ra=saved_header["RA_TRGT"], dec=saved_header["DEC_TRGT"], unit="deg", frame='icrs')
    ...
    if LEDs == 0:
        ...
        led_return = solve_leds(event_path, w, target_source, target_X, target_Y, scale)
